...
Controlled by environment variables:
    CLAUDE_STUB_SCENARIO: success | error | rate_limit | timeout (default: success)
...
def main(argv: list[str] | None = None) -> None:
    ...
    _ = parser.parse_args(argv)  # validate args match real CLI interface
    ...
    if scenario not in _SCENARIO_TO_FIXTURE:
        print(
            f"Stub warning: unknown scenario {scenario!r}, falling back to simple-response.jsonl",
            file=sys.stderr,
        )
    fixture_name = _SCENARIO_TO_FIXTURE.get(scenario, "simple-response.jsonl")
